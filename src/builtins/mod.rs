...
pub fn call_builtin(name: &str, args: Vec<crate::value::Value>) -> crate::error::JingResult<crate::value::Value> {
    match crate::registry::get_builtin(name) {
        Some(builtin) => {
            if args.len() != builtin.arity() {
                return Err(crate::error::JingError::runtime_error(
                    format!("Function '{}' expects {} arguments, got {}", name, builtin.arity(), args.len())
                ));
            }
            builtin.call(args)
        }
        None => Err(crate::error::JingError::runtime_error(
